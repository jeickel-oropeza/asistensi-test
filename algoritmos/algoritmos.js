...
function findInObj(arr, key, searchValue){
    let filteredList = arr.filter(item => {
        return key in item && item[key] == searchValue;
    });
...
function vowelCount(str){
    ...
    str.split('').filter(item => {
        if(vowels.indexOf(item.toLowerCase()) >= 0) {
            repeatVowels.push(item.toLowerCase());
            // totalVowels[item.toLowerCase()] = 1;
        }
    });
    let x = repeatVowels.filter((item, pos, self) => {
